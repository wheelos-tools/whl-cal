...
def draw_registration_result(
    ...
    if (
        ...
        and hasattr(registration_result, "correspondence_set")
        and len(registration_result.correspondence_set) > 0
    ):
        ...
        valid_mask = (
            (corr[:, 0] >= 0)
            & (corr[:, 0] < len(source_points))
            & (corr[:, 1] >= 0)
            & (corr[:, 1] < len(target_points))
        )
        ...
        if len(corr) > 0:
            ...
            matched_source_points_trans = (
                transformation @ matched_source_points_h.T
            ).T[:, :3]

            line_points = np.vstack(
                (matched_source_points_trans, matched_target_points)
            )
            line_indices = [
                [i, i + len(matched_source_points_trans)]
...
def preprocess_point_cloud(
    ...
    if not mask.all():
        logging.info(
            "     Cleaning invalid points: removed %d NaN/Inf points", (~mask).sum()
        )
    ...
    if remove_ground:
        ...
        plane_model, inliers = pcd_filtered.segment_plane(
            distance_threshold=plane_dist_thresh, ransac_n=3, num_iterations=1000
        )
        a, b, c, _ = plane_model
        ...
        if abs(normal[2]) >= ground_normal_threshold:
            ...
            logging.info(
                "     After ground removal: %d points", len(pcd_no_ground.points)
            )
        else:
            logging.info(
                "     Skipped ground removal: dominant plane is not ground-like"
            )
    ...
    if remove_walls:
        ...
        for i in range(max_wall_planes):
            ...
            if abs(normal[2]) < wall_angle_threshold:
                logging.info(
                    "    - Removed wall plane %d, points: %d", i + 1, len(inliers)
                )
    ...
    logging.info(
        "  -> Original points: %d, after preprocessing: %d",
        len(pcd.points),
        len(pcd_no_ground.points),
    )
...
def perform_coarse_registration(
    source_pcd, target_pcd, source_fpfh, target_fpfh, voxel_size: float
):
...
def perform_icp_registration(
    ...
    criteria = o3d.pipelines.registration.ICPConvergenceCriteria(
        relative_fitness=1e-7, relative_rmse=1e-7, max_iteration=max_iteration
    )
    if method == 1:
...
def perform_multistage_refinement(
    ...
    for stage_index, voxel_size in enumerate(stage_voxels, start=1):
        logging.info(
            "--- Refinement Stage %d/%d: voxel size %.3f m ---",
            stage_index,
            len(stage_voxels),
            voxel_size,
        )
        ...
        if len(source_stage.points) == 0 or len(target_stage.points) == 0:
            logging.error(
                "Refinement stage %d produced an empty point cloud.", stage_index
            )
        ...
        logging.info(
            "  -> Max correspondence distance %.3f m", max_correspondence_distance
        )
...
def calibrate_lidar_extrinsic(
    ...
    if initial_transform is None:
        ...
        target_fpfh = compute_fpfh_features(
            target_preprocessed, preprocessing_params["voxel_size"]
        )
        ...
        logging.info(
            "Coarse registration matched points: %d",
            len(initial_guess.correspondence_set),
        )
    ...
    if not np.isfinite(initial_guess_transform).all():
        logging.error(
            "Coarse registration failed, invalid initial transformation matrix."
        )
        return None, None, None

    logging.info("--- Step 4: Fine Registration (Multi-stage Refinement) ---")
    registration_result, source_final_stage, target_final_stage = (
        perform_multistage_refinement(
            source_cloud,
            target_cloud,
    ...
    quat_final, trans_final = matrix_to_quaternion_and_translation(
    ...
    if is_draw_registration:
        if initial_guess is not None:
            ...
            draw_registration_result(
                ...
                target_preprocessed,
                initial_guess_transform,
                window_name="Coarse Registration",
