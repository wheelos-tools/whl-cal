...
def collect_pointcloud_metadata(
    record_files: Iterable[str], topics: Iterable[str]
) -> dict[str, list[PointCloudMeta]]:
    ensure_record_available()
...
def pointcloud_message_to_open3d(msg) -> o3d.geometry.PointCloud:
    if hasattr(msg, "points_xyz_array"):
        ...
        points = np.array(
            [(point.x, point.y, point.z) for point in msg.point], dtype=np.float64
        )
    if points.size == 0:
...
def find_synchronized_pairs(
    source_metas: list[PointCloudMeta],
    target_metas: list[PointCloudMeta],
    max_delta_ns: int,
    max_pairs: int,
) -> list[tuple[PointCloudMeta, PointCloudMeta, int]]:
    ...
    for source_meta, target_meta, delta_ns in candidates:
        if (
            source_meta.timestamp_ns in used_sources
            or target_meta.timestamp_ns in used_targets
        ):
            continue
...
def save_transform_edges_to_dir(
    ...
    saved_paths = []
    for edge in sorted(
        edges, key=lambda item: (item.parent_frame, item.child_frame, item.source_topic)
    ):
...
def merge_transform_edges(
    primary_edges: Iterable[TransformEdge], secondary_edges: Iterable[TransformEdge]
) -> list[TransformEdge]:
...
def lookup_transform(
    graph: dict[str, dict[str, np.ndarray]], source_frame: str, target_frame: str
) -> np.ndarray | None:
...
def analyze_pointcloud_roots(
    pointcloud_frames: Iterable[str], edges: Iterable[TransformEdge]
) -> dict:
    ...
    disconnected_frames = sorted(
        frame for frame, neighbors in adjacency.items() if not neighbors
    )
...
def find_missing_transform_frames(
    graph: dict[str, dict[str, np.ndarray]],
    base_frame: str,
    pointcloud_frames: Iterable[str],
) -> list[str]:
    missing = []
    for frame in sorted(
        {frame for frame in pointcloud_frames if frame and frame != base_frame}
    ):
...
def voxel_overlap_ratio(
    ...
    source_voxels = np.unique(
        np.floor(source_points / voxel_size).astype(np.int64), axis=0
    )
    target_voxels = np.unique(
        np.floor(target_points / voxel_size).astype(np.int64), axis=0
    )
